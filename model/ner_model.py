...
class NERModel(Model):
    ...
    def get_feed_dict(self, words, labels=None, lr_rate=None, drop_out=None):
        ...
        with tf.variable_scope("words"):
            if self.config.use_pretrained is None:
                ...
                word_embeddings = tf.nn.embedding_lookup(_word_embeddings,
                                                         self.w_id, name="word_embeddings")
            else:
    ...
    def add_logits_op(self):
        ...
        with tf.variable_scope("proj"):
            W = tf.get_variable("W", dtype=tf.float32,
                    shape=[2*self.config.hidden_size_lstm, self.config.num_tag])

            b = tf.get_variable("b", shape=[self.config.num_tag],
    ...
    def add_pred_op(self):
        if not self.config.use_crf:
            self.label_pred = tf.cast(tf.argmax(self.logits, axis=-1),
                    tf.int32)

    def add_loss_op(self):
        if self.config.use_crf:
            log_likelihood, trans_params = tf.contrib.crf.crf_log_likelihood(
                    self.logits, self.label, self.seq_len)
    ...
    def build(self):
        ...
        self.add_train_op(self.config.method, self.lr_rate, self.loss,
                self.config.clip)
        self.initialize_session() # now self.session is defined and vars are init

    def predict_batch(self, words, return_feed=False):
        fd, seq_len = self.get_feed_dict(words, drop_out=1.0)
    ...
    def run_epoch(self, train, dev, epoch):
        ...
        for i, (words, labels) in enumerate(minibatches(train, batch_size)):
            fd, _ = self.get_feed_dict(words, labels, self.config.lr_rate,
